...
public class Composite {
    ...
    public void generate_1(int num_segments){
        ...
        for (int i = 0 ; i < num_segments; i++){
            ...
            //comp_container.add(new Quadrat(element_center_x, element_center_y, 2 * element_radius * (float)Math.sin(Math.PI/4.0),2* element_radius * (float)Math.sin(Math.PI/4.0), true));
        }
    }
    ...
    public void generate_2(int num_segments){
        ...
        float element_center_y;
        float angle = (float)(2* Math.PI/ num_segments);
        float radius = diameter/2 * 0.45f;
        for (int i = 0 ; i < num_segments; i++){
            element_center_x = center_x + (float)(Math.cos(i * angle)* radius);
            element_center_y = center_y + (float)(Math.sin(i * angle)*radius);
            comp_container.add(new Kreis(element_center_x, element_center_y, radius, true));
            //comp_container.add(new Quadrat(element_center_x, element_center_y, 2 * radius * (float)Math.sin(Math.PI/4.0),2* radius * (float)Math.sin(Math.PI/4.0),  true));
        }
    }
